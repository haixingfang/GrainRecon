function [Nr_simu,Nr_intersect,Icorr,Iexp_norm,dis_median,SimuSpots,HittedSpots]=index_verify_grey(U,proj,proj_bin_bw,pos,rot_angles,S,B,Ahkl,nrhkl, ...
...
PSF_flag=1;
if PSF_flag==1
    % pixelnr=9/20*2.355*sqrt(2)*(Lsam2det/Lsam2sou)*VoxSize/(2*mean([pixelysize pixelzsize]));
    % PSF=fspecial('Gaussian',round(2*pixelnr)+1,pixelnr/(9/20*2.355*sqrt(2))); % point spread function
    ...
    if ~isempty(select3)
        ...
        for j=1:length(select3)
            ...
            if PSF_flag==1
                ...
                for k1=1:size(PSF,2)
                        for k2=1:size(PSF,1)
                            if (0 < round(k1-1+dety(select3(j))-size(PSF,2)/2)) && (round(k1-1+dety(select3(j))-size(PSF,2)/2) <= detysize) ...
                                    && (0 < round(k2-1+detz(select3(j))-size(PSF,1)/2)) && (round(k2-1+detz(select3(j))-size(PSF,1)/2) <= detzsize)
                                Iexp=Iexp+double(proj{i}(round(k2-1+detz(select3(j))-size(PSF,1)/2), ...
                                    round(k1-1+dety(select3(j))-size(PSF,2)/2)))*PSF(k2,k1);
                            end
                        end
                end
            else
...
Iexp_norm=Iexp_sum/sqrt(Iexp_sum2);
