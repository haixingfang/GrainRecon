% run jobs for reconstruction subvolumes
% Jan 3, 2022
function run_GrainMapping_subvol_fun(subvolnumber)
...
sprintf('Launching grain mapping for subvolume %d ...',subvolnumber)

if strcmp(compute_opt,'gpu_cuda_index_compete') == 1
    GrainMapping_index_and_compete(OutputFolder,fname_prefix,RecVolumePixel,SampleName);
else
    GrainMapping_fun(OutputFolder,fname_prefix,RecVolumePixel,SampleName,compute_opt);
end
