...
fprintf("Before revising: un-indexed fraction = %d / %d = %.3f\n",length(find(DS_merge.Mask==1))-length(find(DS_merge.GrainId>0)), ...
...
if ~gpu_avail==1
    fprintf("Use cpu for filling indexing ...\n")
    fprintf("Only consider voxels belonging to small grains ...\n")
    ...
    fprintf("Use gpu_cuda for filling indexing ...\n")
    fprintf("Consider voxels with low completeness or those belonging to small grains ...\n")
    [DS_merge] = revise_all_unindexed_voxel_cuda(DS_merge,proj_bin_bw,rot_angles,S,B,Ahkl,nrhkl, ...
                ...
                RecVolumePixel,tomo_scale,VoxSize,simap_data_flag,1);
end

if (length(find(DS_merge.Mask==1))-length(find(DS_merge.GrainId>0))<=30000 || gpu_avail==1) && revise_all_flag==1
