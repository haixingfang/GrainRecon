...
for jj=1:nr
    ...
    dety=round(A_gr(jj,17));
    detz=round(A_gr(jj,18));
...
%                     detz = randi([detz-round(pixelnr/2),detz+round(pixelnr/2)],1,1);
        ...
        else                    % Anisotropic Gaussian type peak
...
%                 label_pos = [label_pos;detysize-mean(A_filted(:,17)) detzsize-mean(A_filted(:,18))];
                label_pos = [label_pos;mean(A_filted(:,17)) mean(A_filted(:,18))];   % modified on Sep 10, 2021
            end
        end
        end
    end
