% grain verification
% June 14, 2021
function [Nr_simu,Nr_intersect,dis_median,SimuSpots,HittedSpots]=forward_comp_pair_spots(U,proj_bin_bw,Spots,pos,rot_angles,S,B,Ahkl,nrhkl, ...
...
%                         rot_angles(i) Spots{i}(spotpair_exp_ID,9:10) minDis dety(select3(j)) detz(select3(j)) dis];
                    HittedSpots=[HittedSpots;reshape(U,1,9) pos hkl(:,select3(j))' Energy_hkl(select3(j)) ...
                        rot_angles(i) Spots{i}(spotpair_exp_ID,9:10) Spots{i}(spotpair_exp_ID,11) dety(select3(j)) detz(select3(j)) dis];
                end
            end
