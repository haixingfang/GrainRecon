...
% ImageFolder='F:\dtu_work\HomeDrive_M_20210321\MetaData\Projections_Dss14_Dsd14\Projections_16';
% name1='proj';
% name3='.tiff';
% Nrolling=11;
% Background=5; % [0 6] 
...
% CombineFlag=1;
% if CombineFlag==1
%     CombineFlag_threshold=160; % define a value for binary thresholding
% end
% write_im=0; % save segmented image
...
if nargin<=12
    CombineFlag=0;
    CombineFlag_threshold=255; % binary segmentation
end
...
%     im_spot_bin=LoG_segmentation_parallel_split(imdata,Background,sigma,percent,MinSpotSize);
    
%     im_spot_bin(BeamStopY(1):BeamStopY(2),BeamStopZ(1):BeamStopZ(2))=0;
    im_spot_bin=dip_image(im_spot_bin,'bin');
    if CombineFlag==1
        im_bin=im>CombineFlag_threshold;
...
%     MajorAxes - principal axes of an object
    ...
    if write_im==1 % write images
        ...
        imwrite(uint16(im_label),sprintf('%s/%s%0.4d.tif',OutputFolder,'label',i),'tif');
    end
