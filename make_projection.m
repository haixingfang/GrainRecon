function [SpotsPair,GrainIndex,GrainIndex_unique]=make_projection(A,imdata,Spots,SubGrain,hkl_square,hkl_color, ...
...
%     [SpotBoundary,frame_label,~,~] = bwboundaries(frame_bin,'noholes');
    ...
    for kk=1:length(frame_measure)
        frame_measure_list(kk,:)=[frame_measure(kk).Area frame_measure(kk).Centroid frame_measure(kk).BoundingBox ...
...
%         end
        ...
        if ~isempty(spotpair_exp_ID) && A_filted(1,4)^2+A_filted(1,5)^2+A_filted(1,6)^2<=hkl_square(3)
                % && A_filted(1,22)>=7 && A_filted(1,22)<=20 % spot energy should be in the range [7 30]
...
%                         frame_measure_list(pair_spot_ID,1) Spots{rot_number}(spotpair_exp_ID,11)];
            SpotsPair=[SpotsPair;A_filted(1,2) grain_centroid(A_filted(1,2),:) A_filted(1,4:6) ...
                    A_filted(1,8:10) A_filted(1,22) rot ...
                    frame_measure_list(pair_spot_ID,2:3) spotpair_exp_ID Spots{rot_number}(spotpair_exp_ID,9:10) ...
                    frame_measure_list(pair_spot_ID,2)-Spots{rot_number}(spotpair_exp_ID,9) ...
                    frame_measure_list(pair_spot_ID,3)-Spots{rot_number}(spotpair_exp_ID,10) ...
...
%     imwrite(frame_image,filename,'tif'); % Write out tiff file
    ...
    if ~isempty(SpotsPair)
        ...
        SpotsPair_unique=make_unique_spot_pair(SpotsPair);
        for i=1:length(SpotsPair_unique(:,1))
            if (round(SpotsPair_unique(i,14))>2 && round(SpotsPair_unique(i,14))<detzsize-2 && ...
                round(SpotsPair_unique(i,13))>2 && round(SpotsPair_unique(i,13))<detysize-2 && ...
                round(SpotsPair_unique(i,17))>2 && round(SpotsPair_unique(i,17))<detzsize-2 && ...
                round(SpotsPair_unique(i,16))>2 && round(SpotsPair_unique(i,16))<detysize-2)
                imdata_cl_track(round(SpotsPair_unique(i,14))-2:round(SpotsPair_unique(i,14))+2, ...
                    round(SpotsPair_unique(i,13))-2:round(SpotsPair_unique(i,13))+2,1)=255;
                imdata_cl_track(round(SpotsPair_unique(i,14))-2:round(SpotsPair_unique(i,14))+2, ...
                    round(SpotsPair_unique(i,13))-2:round(SpotsPair_unique(i,13))+2,2)=0;
                imdata_cl_track(round(SpotsPair_unique(i,14))-2:round(SpotsPair_unique(i,14))+2, ...
                    round(SpotsPair_unique(i,13))-2:round(SpotsPair_unique(i,13))+2,3)=0;   % simulated spots

                imdata_cl_track(round(SpotsPair_unique(i,17))-2:round(SpotsPair_unique(i,17))+2, ...
                    round(SpotsPair_unique(i,16))-2:round(SpotsPair_unique(i,16))+2,1)=0;
                imdata_cl_track(round(SpotsPair_unique(i,17))-2:round(SpotsPair_unique(i,17))+2, ...
                    round(SpotsPair_unique(i,16))-2:round(SpotsPair_unique(i,16))+2,2)=0;
                imdata_cl_track(round(SpotsPair_unique(i,17))-2:round(SpotsPair_unique(i,17))+2, ...
                    round(SpotsPair_unique(i,16))-2:round(SpotsPair_unique(i,16))+2,3)=255; % exp spots
            end
        end
